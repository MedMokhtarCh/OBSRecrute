...
const applicationSlice = createSlice({
  ...
  reducers: {
    ...
    requestForApplicationsByJobId(state, action) {
      state.loading = true;
      state.error = null;
    },
    successForApplicationsByJobId(state, action) {
      state.loading = false;
      state.error = null;
      state.applications = action.payload;
    },
    failureForApplicationsByJobId(state, action) {
      state.loading = false;
      state.error = action.payload;
    },
    requestForUpdateStatus(state, action) {
      state.loading = true;
      state.error = null;
      state.message = null;
    },
...
export const fetchEmployerApplications = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `http://localhost:4003/api/v1/Application/employer/getall`,
      {
  ...
  async (dispatch) => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost:4003/api/v1/application/updateStatus/${id}`,
        { status },
        { withCredentials: true }
      );

      dispatch(
        applicationSlice.actions.successForUpdateStatus({
...
export const fetchApplicationsByJobId = (jobId) => async (dispatch) => {
  dispatch(applicationSlice.actions.requestForApplicationsByJobId());
  try {
    const response = await axios.get(
      `http://localhost:4003/api/v1/Application/applications/job/${jobId}`,
      { withCredentials: true }
    );
    dispatch(
      applicationSlice.actions.successForApplicationsByJobId(
        response.data.applications
      )
    );
    dispatch(applicationSlice.actions.clearAllErrors());
    ...
    dispatch(
      applicationSlice.actions.failureForApplicationsByJobId(
...
export const fetchJobSeekerApplications = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `http://localhost:4003/api/v1/Application/jobseeker/getall`,
      {
        withCredentials: true,
      }
    );
...
export const postApplication = (data, jobId) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `http://localhost:4003/api/v1/Application/post/${jobId}`,
      data,
      {
        withCredentials: true,
...
export const deleteApplication = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `http://localhost:4003/api/v1/Application/delete/${id}`,
      { withCredentials: true }
    );
    dispatch(
